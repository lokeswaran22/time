    ...
    try {
        ...
        try {
            await run(
                ...
                [adminUser, adminPass, 'admin', new Date().toISOString()]
            );
...
app.post('/api/login', async (req, res) => {
    ...
    console.log(`Login attempt for: '${username}' with password length: ${password ? password.length : 0}`);
    ...
    try {
        ...
        if (!user) {
            ...
            const debugUser = await get('SELECT * FROM users WHERE username = ?', [username]);
            if (debugUser) {
                console.log(`Debug: User found but password mismatch. Stored: '${debugUser.password}' vs Received: '${password}'`);
            } else {
